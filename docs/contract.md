...
The archive transport requires bucket listing/location and multipart-upload permissions in addition to object reads and writes. The [target-cache policy example](target-cache-policy.json) supplies the full set for the `rust-target-v1-` prefix. Add it alongside existing compiler-cache permissions after replacing the example bucket name.
