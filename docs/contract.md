...
`setup-rust` exposes `target-cache-key`, `target-cache-paths`, `target-cache-endpoint`, `target-cache-port`, `target-cache-insecure`, `target-cache-save-allowed`, and `target-cache-hit` for an explicit S3 save step after failed checks. These outputs contain no credentials. Callers must require `target-cache-save-allowed == 'true'` and should skip an exact hit. Automatic uploads remain success-only. Explicit cache saving does not mark failed checks successful or attest to the validity of build outputs.
