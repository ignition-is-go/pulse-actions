...
`cache-target-backend` accepts `github` or `s3` and defaults to `github`. It is available on `setup-rust` and all three Rust reusable workflows. The selected backend stores Cargo's registry/git cache, the optional Windows sparse index, and enabled target directories; `cache-targets` continues to control only whether build directories are cached. `cache-save-if: 'false'` makes target caching restore-only for either backend. S3 requires a complete static compiler-cache configuration; ambient and anonymous modes are not supported for archives. `setup-rust` and the reusable workflows retain zstd's level 3 default for S3 target archives and accept levels 1 through 19 through `cache-zstd-level`. S3 uses staged archive transfers by default. Linux callers can select the streaming provider with `cache-s3-transfer: streaming`; other platforms reject that mode. The target transfer mode participates in the cache identity so staged and streaming target archives have separate cache entries. Cargo registry/git archives remain staged and use a separate lockfile-based key.
