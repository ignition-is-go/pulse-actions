...
The optional Windows sparse-index cache still uses GitHub storage. S3 archive failures appear in job logs and do not fall back to GitHub. Missing credentials fail setup when S3 caching is explicitly selected. The default backend remains `github`.
